use pinocchio::{
    account_info::AccountInfo, entrypoint, instruction::{Account, AccountMeta, Instruction}, msg, program::{invoke, invoke_signed, invoke_unchecked}, pubkey::Pubkey, ProgramResult
};
...
pub fn process_instruction(
    ...
    let accs: Vec<&AccountInfo> = accounts
        .iter()
        .map(|acc| acc)
        .collect();
